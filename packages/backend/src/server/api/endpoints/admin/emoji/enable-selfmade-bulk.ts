...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			...
			const license = `@${me.username}`;

			const nonLicenseEmojiIds = await this.customEmojiService.filterNonLicenseEmojiIds(ps.ids);
			await this.customEmojiService.setLicenseBulk(nonLicenseEmojiIds, license);

			await this.customEmojiService.setSelfMadeBulk(ps.ids, true);
		});
	}
}
