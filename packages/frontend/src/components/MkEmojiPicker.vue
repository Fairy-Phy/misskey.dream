	...
	<div ref="emojisEl" class="emojis" tabindex="-1">
		...
		<div v-once class="group">
			...
			<XSection
				...
				:initialShown="false"
				:emojis="computed(() => customEmojis.filter(e => filterCategory(e, child.value)).filter(v => defaultStore.state.displaySensitiveEmoji ? true : !v.isSensitive).map(e => `:${e.name}:`))"
				:disabledEmojis="computed(() => customEmojis.filter(e => filterCategory(e, child.value)).filter(v => defaultStore.state.displaySensitiveEmoji ? true : !v.isSensitive).filter(e => !canReact(e)).map(e => `:${e.name}:`))"
